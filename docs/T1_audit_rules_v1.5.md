# T1_audit_rules v1.5（凍結・2026-08-30）

T1遡及監査版（MirrorTopology_T1_signmap v1.6_audited）のofficial実行を拘束する凍結規則。
...
エンジン層：G07 M unitarity<1e-12／G08 roundtrip<1e-9／G09a–e 対称性射影（§3b）／G10 二点相関等価<1e-10／
...
## 3b. 入力共分散の対称性（v1.5改訂：射影の監査方法を整理）

CMBtopologyの共分散は約17万固有モードの数値積分と転送関数補間で構成されるため，理論上厳密な
2つの対称性——Hermiticity と reality条件 C_{l,−m;l′,−m′}=(−1)^{m+m′} conj(C_{lm;l′m′})——を
積分精度の範囲でしか満たさない（第1接触実測：reality_raw=5.01e-08。この1点の観測は
「小さな数値積分残差と整合的」であり，パイプライン一般の典型値として一般化はしない）。
T2b-2由来の1e-10許容値は解析的に構成した共分散向けであり，数値積分結果には不適切だった。
**閾値を緩めるのではなく**，以下を凍結する。

**方針**：読み込み時に共分散を上記2対称性を厳密に満たす部分空間へ直交射影する
（Hermite化と reality像との平均。両involutionはFrobenius内積について直交かつ可換）。
covarianceファイル自体は改変せず（raw file SHAは生成物のまま），射影は読み込み時のみ行う。

**G09（下位gateのAND）**：
- **G09a raw symmetry**：herm_raw ≤ 1e-5 かつ reality_raw ≤ 1e-5。
  この上限は「**第1接触の入力検証失敗を受け，科学出力を調べる前に凍結した較正上限**」であり，
  事前登録値ではない（透明性のため明記）。役割は破損検出であって精度の主張ではない。
- **G09b post-projection exactness**：herm_post < 1e-12 かつ reality_post < 1e-12。
- **G09c correction size（射影量の実質的安全gate）**：correction_max_rel < 1e-6 かつ
  correction_fro_rel < 1e-6（再smoke前に凍結）。両値は全点でprovenanceに記録する。
- **G09d quadratic-mean identity**：‖C_real_projected −(C_real_raw+C_real_rawᵀ)/2‖_rel < 1e-12。
  E[Q]=tr(P·C_real)（Pは実対称）ゆえ C_real の反対称部分は恒等的に落ちる。したがって
  **射影は任意の実対称二次統計量の平均を代数的に保存する**——全走査軸を個別に検査する必要は
  なく，この恒等式1本で全ての鏡映・半回転射影について保証される。
- **G09e E[S±] identity（実装整合性のみ）**：射影前後のE[S±]差が機械精度であること。
  これはG09dから代数的に従うため**感度試験ではない**。非ゼロならコード誤りの兆候。

**provenance**：raw file SHA・raw array SHA・**projected complex covariance SHA**・
**projected real covariance SHA**・correction norms・PSD診断（lambda_min_raw・
lambda_clip_max・effective_rank）を全点で記録し，raw→射影→実基底→科学出力の連鎖を
SHAで完全追跡可能にする。
