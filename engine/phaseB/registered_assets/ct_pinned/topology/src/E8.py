...
import numpy as np
from numpy import pi, sqrt, dot, exp, conjugate
...
class E8(Topology):
  def __init__(self, param, debug=True, make_run_folder = False):
    ...
    print('Running - E8 l_max={}_LAx_{}_LAy_{}_LBx_{}_LBz_{}_LCy_{}_x_{}_y_{}_z_{}'.format(self.l_max, 
...
def get_list_of_k_phi_theta(k_max, k_min, LAx, LAy, LBx, LBz, LCy, M_A, M_B, x0):
    ...
    for n_x in range(2, n_x_max + 1, 2):
      ...
      for n_z in range(-n_z_max, n_z_max + 1):
        ...
        for m in range(2):
          tilde_xi[cur_index, 0, m] = exp(- 1j * (k_x * x0[0] + k_z * x0[2])) / sqrt(2)
          tilde_xi[cur_index, 1, m] = (
                                    shortle[m] * exp(-1j * (-k_x * x0[0] + k_z * x0[2]))
                                    * exp(1j * (k_x * LBx + k_z * LBz))
                                    ) / sqrt(2)
        cur_index += 1
    ...
    for n_y in range(1, n_y_max + 1):
      ...
      for n_z in range(-n_z_max, n_z_max + 1):
        ...
        theta[cur_index] = cur_theta
        for m in range(2):
          tilde_xi[cur_index, 0, m] = exp(- 1j * (k_y * x0[1] + k_z * x0[2])) / sqrt(2)
          tilde_xi[cur_index, 1, m] = ( exp(-1j * (-k_y * x0[1] + k_z * x0[2]))
                                    * exp(1j * k_y * LAy)
                                    ) / sqrt(2)
        cur_index += 1
        ...
        for n_z in range(-n_z_max, n_z_max+1):
          ...
          for m in range(2):
            tilde_xi[cur_index, 0, m] = (
                                        exp(-1j * dot(k_vec, x0)) 
                                        + shortle[m%2] * exp(-1j * dot(k_vec, M_A @ M_B @ x0))
                                        * exp(1j * dot(k_vec, M_A @ T_B + T_A))
                                        ) / sqrt(4)
            tilde_xi[cur_index, 1, m] = (
                                        exp(-1j * dot(k_vec, M_A @ x0)) * exp(1j * dot(k_vec, T_A))
                                        + shortle[m%2] * exp(-1j * dot(k_vec, M_B @ x0)) 
                                        * exp(1j * dot(k_vec, T_B))
                                        ) / sqrt(4)
          cur_index += 1
    ...
    print('E8 Final num of elements:', k_amp.size, 'Minimum k_amp', np.amin(k_amp), 'n_x_max', n_x_max, 'n_z_max', n_z_max)
