...
def main():
    ...
    try:
        ...
        DOC_REG = dict(schema="d3_cov_registry_v1", family=fam, partition=R.get("partition"), selection=R.get("selection"), registry_sha256=reg.registry_sha256, config_map_sha256=cm["map_sha256"], case_table_sha256=ct["table_sha256"], env_fingerprint=ENV_FINGERPRINT, cmbtopology_commit=EXPECTED_CMBTOPO_COMMIT, run_config=RUN_CFG, source=R["source"], configurations=registry_out, n=len(registry_out), scope=("SELFTEST subset" if sel else (f"{fam}: 12-position added configurations" + (f" (sizes {sizes_arg})" if sizes_arg else ""))))
        DOC_PC1 = dict(schema="d3_pc1_results_v1", family=fam, partition=R.get("partition"), selection=R.get("selection"), contract=PC1_CONTRACT, case_table_sha256=ct["table_sha256"], config_map_sha256=cm["map_sha256"], env_fingerprint=ENV_FINGERPRINT, source=R["source"], representation=R.get("representation"), used_D=R.get("used_D"), cases=pc1, configuration_status=R.get("pc1_status", {}), with_h2=a.with_h2, scope="asset-layer PC-1 evaluation per the transcribed A11 R2/R5 contract; PC1_FAIL forbids formal consumption of the configuration; no position-state conversion")
        DOCS = {"d3_cov_registry.json": DOC_REG, "d3_pc1_results.json": DOC_PC1, "d3_partial_evidence.json": evidence, "d3_env_lock.json": R["env_lock"]}
        R["n_bases"] = len(base_C); R["n_cases"] = n_cases
        # publication: every document is written from an in-memory snapshot, then its bytes are read back ONCE and those SAME bytes are hashed, decoded and compared value-level
        try:
            snaps = {name: json.loads(json.dumps(doc, default=str)) for name, doc in DOCS.items()}; pub = {}; ok_ev = True
            for name, doc in DOCS.items():
                atomic_write_json(doc, os.path.join(a.out, name)); b = open(os.path.join(a.out, name), "rb").read(); back = json.loads(b.decode("utf-8")); pub[name] = dict(sha256=hashlib.sha256(b).hexdigest(), bytes=len(b), value_level_readback_ok=(back == snaps[name]))
                ok_ev = ok_ev and back == snaps[name]
            ok_ev = ok_ev and snaps["d3_partial_evidence.json"]["status"] == "COMPLETE" and snaps["d3_env_lock.json"]["env_fingerprint"] == ENV_FINGERPRINT and set(snaps["d3_cov_registry.json"]["configurations"]) == set(registry_out) and set(snaps["d3_pc1_results.json"]["cases"]) == set(pc1)
            R["published_evidence"] = dict(documents=pub, value_level_readback_ok=bool(ok_ev)); G["G_evidence_saved"] = bool(ok_ev)
            if not ok_ev: R["failures"].append("published evidence differs from the computed snapshots (value-level read-back of the same bytes)")
        except Exception as ex_: G["G_evidence_saved"] = False; R["failures"].append(f"evidence publication: {ex_!r}")
