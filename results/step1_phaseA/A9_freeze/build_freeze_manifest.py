...
dep_commit = sys.argv[4] if len(sys.argv) > 4 else None
def sha(p):
...
man = dict(schema='freeze_manifest_v1', stage=label, date=datetime.datetime.now(datetime.timezone.utc).isoformat(),
           git=dict(origin='https://github.com/tsujikeita/mirror-topology', path=f'results/step1_phaseA/{os.path.basename(os.path.abspath(root))}/',
                    archive_tag=tag, archive_commit='resolved externally by archive_tag (git ls-remote ... refs/tags/<tag>^{})',
                    dependency_commit=dep_commit, note='the archive commit is not embedded to avoid self-reference; resolve it from the annotated tag'),
