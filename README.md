...
Step 0 predates the git-gate machinery used by T1 and T2a: its provenance JSON records the
pinned dependency commits, input map hashes, mask and C_ℓ hashes, and the result of every
condition it checked, but **not a hash of the notebook itself**. The correspondence between
`MirrorTopology_Step0_official_v0.7.ipynb` and the four Step 0 artefacts was therefore
established after the fact, by checking that

1. the three pinned commits recorded in the provenance (`d36e7567…`, `63aa853f…`,
   `aaf81374…`) appear verbatim in the notebook source;
2. all four output filenames are written by the notebook's save cell; and
3. every one of the twenty keys in the provenance JSON is generated by that same cell.

See `docs/step0_v0.7_correspondence_check.md` for the full record. Because the notebook and its
artefacts are stored together under `results/step0_v0.7/`, a third party can repeat this check
directly. Step 0 is therefore reproducible, but its source identity rests on this content-level
correspondence rather than on a hash recorded at run time, as it does for T1 and T2a.

The earlier `MirrorTopology_Step0_observed_values_v0.6.ipynb` is superseded by v0.7 and has been
...
- 追試は§3の手順（タグをcheckout→ノートブックを編集せず全実行）。ゲートが停止したら，
  それは設計どおりの動作です。
- Step 0はgit gate導入前の解析のため，**ノートブック自身のハッシュがprovenanceに記録されて
  いません**。ノートブックと成果物の対応は事後の内容照合で確認しており，その記録は
  `docs/step0_v0.7_correspondence_check.md`にあります（§6）。
